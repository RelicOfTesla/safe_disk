...
typedef ProgressCallback = void Function(int currentBytes, int totalBytes, int percent);
...
class IncrementalEncryptor {
  ...
  Future<bool> addBlock(Uint8List data) async {
    ...
    try {
      ...
      final resultStr = _nativeLib.incrementalEncryptorAddBlock(_handleID!, dataBase64);
...
class IncrementalDecryptor {
  ...
  Future<Uint8List?> decryptBlock(int blockIndex) async {
    ...
    try {
      final resultStr = _nativeLib.incrementalDecryptorDecryptBlock(_handleID!, blockIndex);
  ...
  Future<Uint8List?> decryptRange(int offset, int length) async {
    ...
    try {
      final resultStr = _nativeLib.incrementalDecryptorDecryptRange(_handleID!, offset, length);
      final result = IncrementalBlockResult.fromJson(resultStr);
      
      if (!result.success) {
        onError?.call(result.error ?? 'Failed to decrypt range $offset-$length');
        return null;
      }
  ...
  Future<bool> verifyBlockIntegrity(int blockIndex) async {
    ...
    try {
      final resultStr = _nativeLib.incrementalDecryptorVerifyBlockIntegrity(_handleID!, blockIndex);
      final result = FFIResult.fromJson(resultStr);
  ...
  Future<bool> verifyIntegrity() async {
    ...
    try {
      final resultStr = _nativeLib.incrementalDecryptorVerifyIntegrity(_handleID!);
      final result = FFIResult.fromJson(resultStr);
      return result.success;
    } catch (e) {
      onError?.call(e.toString());
  ...
  Future<IncrementalBlockInfo?> getBlockInfo(int blockIndex) async {
    ...
    try {
      final resultStr = _nativeLib.incrementalDecryptorGetBlockInfo(_handleID!, blockIndex);
  ...
  Future<List<IncrementalBlockInfo>?> getAllBlockInfo() async {
    ...
    try {
      final resultStr = _nativeLib.incrementalDecryptorGetAllBlockInfo(_handleID!);
