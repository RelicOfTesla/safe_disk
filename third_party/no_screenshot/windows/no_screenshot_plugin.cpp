...
HWND NoScreenshotPlugin::GetFlutterWindowHandle() {
  HWND view_hwnd = registrar_->GetView()->GetNativeWindow();
  if (view_hwnd == nullptr) return nullptr;
  // Flutter's Windows embedding reparents the view HWND as a child via
  // SetParent(). SetWindowDisplayAffinity must target the top-level root
  // window for screenshot tools (PrtSc, WeChat, Snipping Tool, etc.) to
  // observe the protection.
