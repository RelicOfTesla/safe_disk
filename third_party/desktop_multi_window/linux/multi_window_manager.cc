...
// Flutter 3.44's FlView dispose path unconditionally calls RemoveView, even
// for an engine's implicit view. Keep only the destroyed wrapper alive after
// shutting its engine down so that invalid call is never made.
std::vector<GtkWidget*> _retired_implicit_views;
...
std::string MultiWindowManager::Create(FlValue* args) {
  ...
  if (use_header_bar) {
    ...
    gtk_header_bar_set_title(header_bar, config.title.c_str());
  ...
  gtk_window_set_default_size(window, config.width, config.height);
  ...
  g_signal_connect(
      ...
      G_CALLBACK(+[](GtkWidget* widget, gpointer arg) {
        ...
        if (child && FL_IS_VIEW(child)) {
          g_object_ref(child);
          _retired_implicit_views.push_back(child);
          g_object_run_dispose(G_OBJECT(fl_view_get_engine(FL_VIEW(child))));
