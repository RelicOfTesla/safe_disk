...
| UI-65 | 文件界面多选与打开方式 | UX/键盘/鼠标 | 88% | 已完成单击/双击打开设置、默认值与持久化，并接入主页、list/grid；文件和目录均可通过 Ctrl/Meta 点击切换、Shift 范围选择、鼠标主键拖选、list/grid 方向键移动（grid 上下按实际列数跳转）、Home/End 跳到首尾、Ctrl/Meta+A 全选、Shift+方向键/Home/End 扩展和 Ctrl+Space 切换，Esc 可退出选择模式，选择集合仍由 HomePage 单一持有。普通非选择模式下单击/双击目录仍保持打开行为；文件浏览器、主页批量操作和设置事务已有 widget 回归；框选绘制已与虚拟化文件列表解耦，不再在每个指针移动事件重建整棵浏览器；仍缺三平台真实鼠标/键盘、触控和读屏验收，以及真实大目录拖选帧率/内存验收。 |
