...
| ID | 任务 | 类型 | 当前进度 | 验收条件 |
|---|---|---|---:|---|
| UI-39 | 应用隐藏时的安全自动锁定 | 安全/生命周期 | 78% | 设计见 [AUTO_LOCK_DESIGN.md](design/AUTO_LOCK_DESIGN.md) 与 [AUTO_LOCK_CONTENT_WINDOW_PROTOCOL.md](design/AUTO_LOCK_CONTENT_WINDOW_PROTOCOL.md)。设置开关已持久化；HomePage 监听 `hidden`/`paused`，对无内容窗口、无脏文档、无活动写入的 root 依次释放当前目录 cursor、关闭 native root、清除该 root 的应用内剪贴板/broker 并保留侧边栏历史。恢复可见后会提示锁定/跳过/失败摘要。widget 覆盖开关保存、默认关闭、单 root 锁定并重新验证、两个已解锁 root 逐一关闭且恢复后不泄露旧条目，以及内容窗口阻断锁定；broker/controller 覆盖脏记事本和活动写入的关闭判定。主动关闭带子窗口 root 时，bridge 会先撤销该 root 的 token、拒绝迟到 RPC，等待已接受的保存完成后再关闭原生窗口；定向测试覆盖该顺序。含内容窗口自动锁定需要子窗口草稿确认协议，当前单向 channel 不具备主窗口请求能力，不能以直接关闭替代；重新解锁竞争、真实 FFI 与三平台 `hidden`/`paused` 验收仍缺。per-root TTL 见 UI-44。 |
