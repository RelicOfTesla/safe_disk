...
| UI-59 | 真实 FFI 图片集成测试本地化壳 | 测试/回归 | 90% | `native_ffi_integration_test.dart` 的 WebP 用例现以应用相同的 locale、delegates 和 supportedLocales 构建 `SecureImageViewer`，不再因缺少 `AppLocalizations` 触发空值断言；该真实 FFI 单用例已通过。完整真实 FFI 集成文件的修复后复跑仍缺可记录的结果：本轮审批超时而未执行完成，不能据此标为 100%。 |
