...
| UI-31 | Ubuntu 新窗口记事本关闭闪退回归 | P0 Bug/平台/稳定性 | 45% | 真实基线和修复基点固定为 `f63d1e5b01be3a0bdc967f9108709e33a42b3e47`；运行基线时必须用绝对 `SAFE_DISK_FFI_LIBRARY` 指向该 worktree 自身编译库，避免旧版 bundle 漏拷贝 `.so` 导致伪“密码错误”。复现路径：主窗口侧边栏打开 `ff` root，密码 `123`，目录内文件右键“新窗口打开”，等待数秒，点击子窗口标题栏 X，继续观察 5 秒；旧版表现为主进程崩溃。此前虚拟文档 + RPC 主动关闭 smoke 不覆盖 FFI 解密、右键入口、停留时间和用户标题栏关闭，不能作为修复完成证据。当前候选修改尚未定案，必须先在该基线按真实路径留存日志，再只改变一个变量做 A/B，最后收缩为最小补丁；完成标准是同一路径连续多轮无主进程退出、GLX BadAccess、无效 messenger 或 lease 泄漏 |
