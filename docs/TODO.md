...
| UI-43 | 增量目录的筛选与排序边界 | UX/正确性 | 90% | 未完成 cursor 的 list/grid 现保持 walker 页顺序，不再对局部页排序；工具栏显示已加载项并禁用排序。筛选框提示“仅筛选已加载条目”，无匹配时不会宣称整个目录为空，且可手动继续加载下一页；产品明确不在输入筛选词时自动枚举整个目录，避免宽目录输入造成不可预期阻塞；EOF 后恢复既有排序。widget 覆盖 list/grid 页顺序、筛选范围/空态、继续加载和完成后排序菜单；真实 FFI 全量 Flutter 回归通过。`tool/benchmark_directory_cursor.dart` 提供不进 CI 的 name-encrypted 大目录基准，已在 Linux 用真实动态库完成 100000 条直系文件、200 条页大小的创建/首屏/全量分页/RSS 采样记录，见 [DIRECTORY_PAGINATION_DESIGN.md](design/DIRECTORY_PAGINATION_DESIGN.md)。该记录不等同 Flutter 滚动帧或其它设备结论；仍缺跨平台验收。 |
...
| UI-60 | 解锁期间切换目录的会话一致性 | Bug/安全/异步状态 | 90% | 解锁启动时捕获目标目录实例和 path；native root 打开后异步检查 unfinished transfer 返回时，若当前目录实例已改变，就只关闭刚打开的 root 并退出，不改当前选择、列表、剪贴板或 idle 状态。widget 使用可控 Future 覆盖 A 解锁期间切换至 B：A session 被关闭、B 保持锁定且不加载内容。`flutter analyze --no-pub` 和完整真实 FFI Flutter 回归通过。仍缺 Linux/Windows/macOS 真实侧边栏切换与窗口/进程异常关闭的桌面验收。 |
| UI-64 | 主界面复制/剪切快捷键 | UX/键盘 | 90% | `Ctrl/Cmd+C` 与 `Ctrl/Cmd+X` 已接入主界面快捷键：优先复用当前选中项，没有多选时使用最近的键盘/右键目标，无目标时安全忽略；复制/剪切继续使用安全文件剪贴板，不接触系统文本剪贴板。widget 已覆盖选中项复制、剪切和剪贴板状态。仍缺三平台真实键盘布局与读屏验收。 |
| UI-65 | 文件界面多选与打开方式 | UX/键盘/鼠标 | 85% | 已完成单击/双击打开设置、默认值与持久化，并接入主页、list/grid；文件和目录均可通过 Ctrl/Meta 点击切换、Shift 范围选择、鼠标主键拖选、list/grid 方向键移动（grid 上下按实际列数跳转）、Home/End 跳到首尾、Ctrl/Meta+A 全选、Shift+方向键/Home/End 扩展和 Ctrl+Space 切换，Esc 可退出选择模式，选择集合仍由 HomePage 单一持有。普通非选择模式下单击/双击目录仍保持打开行为；文件浏览器、主页批量操作和设置事务已有 widget 回归；仍缺三平台真实鼠标/键盘、触控和读屏验收，以及大目录拖选性能验收。 |
| UI-66 | 多语言换行转义一致性 | i18n/正确性 | 90% | `appVersionDescription` 与 `noOpenedDirectories` 已从字面量 `\\n` 修正为真实换行；查找提示中的 `\\n` 保留为用户可输入的换行语法说明。中英文 ARB 已重新生成，设置页和侧边栏均有真实换行渲染断言；仍缺其它平台字号/截断实测。 |
| UI-67 | 设置界面重新分类与响应式排版 | UX/可访问性 | 90% | 设置项已按外观、行为、安全、记事本和关于分类；宽屏使用 2×2 卡片并将关于信息置于下方，窄屏单列滚动，保存按钮可随内容滚动访问；恢复默认设置必须重置全部设置项，包括自动锁定和会话 TTL。中英文标题、保存事务、未保存返回提示和宽窄布局已有 widget 回归；仍缺较大字号与三平台视觉截断实测。 |
