...
| UI-31 | Ubuntu 新窗口记事本关闭闪退回归 | P0 Bug/平台/稳定性 | 80% | 未修复窗口基线为 `af42147`（窗口实验修改已放入 `stash@{0}`），FFI 使用同提交源码构建的 `/tmp/libffi_sec_fs_af42147.so`。2026-07-16 已用 `xdotool` 完整确认：选择 `ff`、密码框出现 3 个掩码字符、点击解锁后显示 `Verified` 和真实文件 `新建文件.txt`、右键“在新窗口中编辑”产生独立子窗口、发送与标题栏 X 等价的 `WM_DELETE_WINDOW`。未修复版关闭后先出现 compositor shader 和 GTK/GDK 销毁警告，约 10 秒后主进程以 `GLX BadAccess` 退出。单变量 A/B 仅在创建 Linux 子 `FlView` 时临时选择软件渲染，不增加 Dart 配置 API、不修改关闭生命周期；同路径连续 5 轮均在关闭后观察 15 秒仍存活且无 `GLX BadAccess`，因此当前主线采用该最小止崩补丁。每轮仍有 `GdkWindow unexpectedly destroyed`、空对象 signal disconnect 和 frame clock 警告，后续必须作为 teardown 顺序问题单独收敛；在该警告解决且用户实机复验前不标记完成。虚拟文档 + RPC 主动关闭 smoke 不覆盖 FFI 解密、右键入口和真实窗口关闭，不能作为完成证据 |
...
| UI-37 | 解锁错误分阶段归类与详细诊断 | P0 Bug/可诊断性/安全 | 75% | FFI 已为无效密码、缺少 verifier、无效配置返回稳定结构化 code，Dart `NativeOperationException` 保留 code；认证 try/catch 已收窄为单独 `openRoot` 调用，仅明确的 invalid-password code 显示“密码错误”，verifier/config 显示“加载配置失败”，未知异常显示“操作失败”并保留阶段和脱敏详情，测试确认即使未知错误文本含 `invalid password` 也不误判。此前 f8 UI 解锁失败的原始异常未记录，因此不作归因。剩余：动态库初始化独立错误页、恢复/list 各自稳定 code、跨 FFI 实际 UI 错误详情 E2E |
