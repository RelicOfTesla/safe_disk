...
| UI-90 | 侧边栏 root 上下移动 | UI/侧边栏 | 90% | 侧边栏 root 支持向上/向下调整顺序，边界项禁用对应动作；顺序持久化，重启后保持，不能改变 root 路径、会话和磁盘内容。右键动作、边界菜单和主页持久化接线已完成，侧边栏/壳层/本地化以及 HomePage 双 root 顺序回归通过；重启恢复和真实桌面操作仍待补测。 |
| UI-91 | 网格模式图标宽度异常 | Bug/文件浏览 | 90% | grid 模式下图标在固定单元格内保持完整可见且居中，不出现只有约半个宽度的图标；list/grid/tree 和不同窗口宽度均需有布局回归。图标已使用固定 `56×56` 容器，窄宽度布局回归通过；真实平台字体/渲染差异仍待验收。 |
