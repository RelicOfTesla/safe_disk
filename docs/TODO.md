...
| UI-59 | 文件条目操作栏窄宽与英文溢出 | Bug/响应式/i18n | 90% | 英文批量删除路径的 widget 验收发现 `_FileItemActionRow` 在约 256px 宽度下发生 `RenderFlex overflow`。现已将标签设为可收缩单行省略，完整文本仍供 Semantics 使用；list/grid 的二级点击复用 HomePage 同一个上下文菜单和该组件。主页中英文批量删除回归通过且无布局异常。仍缺 Linux/Windows/macOS 窄窗口、高 DPI 和读屏实测。 |
