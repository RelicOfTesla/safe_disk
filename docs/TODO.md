...
| UI-43 | 增量目录的筛选与排序边界 | UX/正确性 | 90% | 未完成 cursor 的 list/grid 现保持 walker 页顺序，不再对局部页排序；工具栏显示已加载项并禁用排序。筛选框提示“仅筛选已加载条目”，无匹配时不会宣称整个目录为空，且可继续加载下一页；EOF 后恢复既有排序。widget 覆盖 list/grid 页顺序、筛选范围/空态、继续加载和完成后排序菜单；真实 FFI 全量 Flutter 回归通过。`tool/benchmark_directory_cursor.dart` 提供不进 CI 的 name-encrypted 大目录基准，已在 Linux 用真实动态库完成 100000 条直系文件、200 条页大小的创建/首屏/全量分页/RSS 采样记录，见 [DIRECTORY_PAGINATION_DESIGN.md](design/DIRECTORY_PAGINATION_DESIGN.md)。该记录不等同 Flutter 滚动帧或其它设备结论；仍缺筛选时自动加载的桌面可用性决策与跨平台验收。 |
