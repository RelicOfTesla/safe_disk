...
| UI-54 | 用户可见文案清点与迁移门槛 | i18n/工程治理 | 65% | 已建立 [I18N_STRING_INVENTORY.md](design/I18N_STRING_INVENTORY.md)：当前产品 UI 有 27 个文件、269 个候选构造点，按模块定义审核范围、迁移批次、完成条件和防回退规则。`l10n.yaml` 已以中文 ARB 为模板；设置、应用壳、侧边栏、Root 操作与属性、目录浏览器、目录树、文件/目录右键、冲突确认、创建/路径/删除通用对话框和批量结果对话框的首批文案均已逐项迁移，并有中英文 widget 覆盖。尚未实现基线检查脚本，进度面板、记事本和图片浏览器也尚未逐项分类与迁移；不得把清点文档计为文案翻译完成。 |
