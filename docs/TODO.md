...
| UI-58 | 跨 root 目录剪切/移动 | 文件操作/数据安全 | 10% | 设计见 [CROSS_ROOT_DIRECTORY_MOVE_DESIGN.md](design/CROSS_ROOT_DIRECTORY_MOVE_DESIGN.md)。当前 `sec_copy_entry` 已支持跨 root 递归复制，但 sec/FFI 只有文件删除 `DeleteFile/sec_file_delete`，没有受相对路径约束、拒绝 root、删除失败可诊断的目录删除原语。因此 `SecureEntryMoveService` 明确拒绝跨 root 目录移动，不能仅在 UI 解除限制。后续需实现 sec/FFI/Dart 的安全递归删除：复制成功后才删除源目录；删除失败保留两端并报告部分完成；不得跟随 root 外符号链接、不得删除 root 或目标内祖先，必须有真实 FFI 与 UI 队列/冲突回归。 |
