...
| 拖放 | 10% | 设计见 [DRAG_DROP_DESIGN.md](design/DRAG_DROP_DESIGN.md)。已完成首期插件调研：`desktop_drop` 可接收 Linux/Windows/macOS 的外部文件/目录，适合作为拖入适配层；其不提供可信的跨应用拖出目标目录，且 macOS file promise 可能写入应用临时目录，因此首期继续禁止拖出，只接受可验证的普通绝对路径。当前没有平台实现或依赖。 | 实现外部拖入 controller、现有导入/冲突/Transfer 复用和桌面适配；三平台导入/取消/权限/临时明文泄漏验收；在具备可信目标目录协议前不得实现拖出 |
