...
| UI-80 | 主界面刷新快捷键的平台修正 | Bug/快捷键 UI | 70% | 主界面已将 `F5`、`Ctrl+R` 和 `Cmd+R` 接入同一刷新路径，主页回归覆盖三条快捷键；仍需真实平台键盘和焦点验收。 |
| UI-81 | 主界面筛选快捷键 | Feature/快捷键 UI | 70% | `Ctrl/Cmd+F` 已通过 HomePage 的窄接口打开并聚焦当前目录筛选框，筛选仍只作用于当前已加载目录；文件浏览器与主页回归共 80 项通过，仍需真实平台键盘焦点验收。 |
| UI-83 | 安全记事本编辑输入字节上限 | Bug/编辑器安全 | 70% | 编辑器已接入按 UTF-8 字节数限制的输入格式化器，避免编辑文本、撤销历史和草稿因继续输入无限增长；ASCII、多字节和未超限边界测试已覆盖。仍需真实桌面粘贴/输入法、大文本压力验收；不扩大为未知大小文件的流式读取实现。 |
