...
| UI-70 | 文件界面多选与打开模式复核 | UI/交互 | 80% | Ctrl/Meta、Shift、鼠标拖选、键盘选择及单击/双击打开设置已有代码和 widget 回归；已修复双击模式首次点击不显示高亮的问题，list/grid 首次高亮与第二次打开均有回归。仍需核对文件与目录统一选择、焦点/右键状态、设置持久化和三平台真实输入，确认未把测试替代为完成结论。 |
| UI-71 | 多语言换行转义复核 | Bug/本地化 | 70% | ARB 和主要页面已有换行处理及审计工具；仍需逐项确认换行应渲染为真实换行还是字面量 `\\n`，覆盖中英文生成结果、对话框、提示文本和宽窄布局，避免转义层级错误。 |
| UI-72 | 设置界面分类与响应式排版复核 | UI/设置 | 70% | 设置页已有行为、安全、记事本、外观等分组和宽窄布局测试；仍需按实际产品功能重新检查分组、无效开关、保存/返回交互、长文案和大字号布局，不以已有快照代替完整验收。 |
| UI-73 | 安全记事本小键盘输入异常 | Bug/编辑器 | 10% | 仅登记用户报告的编辑器路径；当前 Linux 真实 `xdotool` 诊断未复现，不能据此修改输入链路。必须先在可复现环境记录键盘事件、文本值、选区和 composing 状态，再设计最小修复与回归测试。 |
| UI-74 | 安全记事本剪贴板监视位置 | UI/记事本 | 90% | 剪贴板监视条已移至编辑器底部，并已有刷新、清空、关闭和短文本布局测试；仍需确认窄/宽窗口、大字号、只读模式、系统剪贴板和读屏语义，避免与顶部粘贴入口重复或遮挡编辑区。 |
