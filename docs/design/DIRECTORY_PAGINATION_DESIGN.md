...
> 状态：2026-07-23，native cursor ABI、Dart session、浏览器和目录树增量加载均已实现并有 FFI/widget 回归；未完成加载时的筛选已明确限定为“已加载条目”，并支持手动继续加载扩大范围。产品决策是不因输入筛选词自动枚举整个目录，避免宽目录输入时产生不可预期的长耗时和内存增长；需要全目录结果时由用户主动继续加载。全局排序与全 root 搜索仍未实现。
...
- 增量模式的全局排序和 10 万条性能基线仍未完成；自动枚举获得完整筛选结果已明确不作为当前版本行为。当前筛选不会伪装成全目录搜索，空结果提供继续加载入口。
