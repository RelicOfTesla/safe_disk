...
状态：已完成（2026-07-23）。此阶段只解决设置项未生效的问题，不实现设备校准。

- 设置页保存“新建目录的默认 KDF 档位”，值只能取 `fast`、`balanced`、`strong`、`maximum` 对应的受控值；损坏或旧的非受控值必须回退到 `balanced`。
- 新建目录对话框从该设置取得初始值；用户仍可在高级参数中为本次创建另行选择。该设置只影响尚未创建的目录，绝不重写已有 root。
- UI 必须称为“新建目录默认派生档位”，并说明“尚未按本机性能校准”；不得以毫秒文本承诺实际耗时，也不得展示校准成功状态。
- 本阶段不新增 sec/FFI 接口、不写入 `sec_kdf_*` 元数据，也不改变三种 KDF 现有的粗略映射。那些工作属于后续真正的校准阶段。

阶段 A 自动化验收已覆盖：设置保存和恢复默认值的事务语义；非法持久化值回退；主页进入创建对话框后获得保存的初始值；用户在对话框中手工覆盖值优先；既有 `CreateRootRequest` 默认值测试继续保留。尚未执行真实设备校准、性能测量或跨平台耗时验收。
