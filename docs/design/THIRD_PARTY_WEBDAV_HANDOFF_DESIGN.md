...
> 状态：仅完成设计。当前没有 WebDAV、系统挂载或第三方编辑实现。
...
用户可点击“挂载到系统”请求平台 WebDAV 挂载，以支持只接受本地路径的工具。该能力是可选项：

- 挂载前提示系统和第三方工具可能缓存明文。
- 平台没有可靠挂载方式时直接提示不支持，不退化为普通明文临时目录。
- 关闭会话时请求卸载；若卸载失败，界面保留警告和重试入口。
...
- 内部编辑与外部编辑冲突时不静默覆盖。
- Linux、Windows、macOS 分别验证 WebDAV 访问、撤销和可选挂载的实际行为；未验证的平台不宣称支持自动挂载。
