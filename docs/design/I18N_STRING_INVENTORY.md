...
| 模块 | 文件 | 当前迁移状态 |
| --- | --- | --- |
| 应用壳与入口 | `pages/home_page.dart`、`widgets/home_shell.dart`、`widgets/welcome_screen.dart`、`widgets/native_library_startup_error.dart` | 原生库与内容窗口启动错误、欢迎页、主壳、侧边栏、导入入口和解锁提示已完成；目录别名对话框的标题、字段和提示已迁移。2026-07-22 静态复核发现 `home_page.dart` 的会话关闭、导入导出、文件剪贴板、新建/重命名/删除及成功/失败提示仍有大量硬编码，不能计为主页已完成 |
