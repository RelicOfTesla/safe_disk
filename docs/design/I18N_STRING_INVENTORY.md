...
| 模块 | 文件 | 当前迁移状态 |
| --- | --- | --- |
| 应用壳与入口 | `pages/home_page.dart`、`widgets/home_shell.dart`、`widgets/welcome_screen.dart`、`widgets/native_library_startup_error.dart`、`services/content_window_host_bridge.dart` | 原生库与内容窗口启动错误、欢迎页、主壳、侧边栏、导入入口和解锁提示已完成；目录别名、自动锁定摘要、root 会话关闭/阻断、改密前关闭、导入导出、文件剪贴板、新建/重命名/删除及成功/失败提示均已迁移。内容窗口 bridge 的无效请求与未支持调用改为稳定协议标识，不向子窗口传递中文展示文案。2026-07-23 审计中该 bridge 已无候选；`home_page.dart` 仍有一个 `Unknown error` 英文 fallback。仍需英文路径专门断言及三平台视觉验收 |
